...
public class HttpManager {
    ...
    public ArrayList<Board> requestByGet(String requestUrl){
        ...
        try {
            ...
            Log.d(TAG,"사이즈 수 "+boardList.size());
    ...
    public void requestByPost(String requestUrl,String param){
        ...
        try {
            ...
            con.setDoOutput(true); //서버로부터 데이터를 출력하려면 true 로 처리해야 함

            buffw = new BufferedWriter(new OutputStreamWriter(con.getOutputStream(),"UTF-8"));
            buffw.write(param); //문자열 전송
            buffw.flush();

            int code =  con.getResponseCode(); //여기서 요청 및 응답처리가 발생
            Log.d(TAG,"등록후 서버로 부터 받은 응답코드" + code);
        } catch (MalformedURLException e) {
            e.printStackTrace();
